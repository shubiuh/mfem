...
int main(int argc, char *argv[])
{
   ...
   assert(mesh.bdr_attributes.Max() == 6);
   ess_bdr = 0;
   int top_marker = 5;
   int bottom_marker = 0;
...
//    b.AddDomainIntegrator(new DomainLFIntegrator(sigma));
   ...
   ParGridFunction sigma_gf(&fespace);
...
//    sigma_gf.Save("sigma.gf");
   ...
   // 9. Solve the system using PCG with hypre's BoomerAMG preconditioner.
   Solver *prec = NULL;
   if (pa)
   {
   ...
   {
      ostringstream mesh_name, sol_name, sigma_name;
      mesh_name << "mesh." << setfill('0') << setw(6) << myid;
      sol_name << "sol." << setfill('0') << setw(6) << myid;
      sigma_name << "sigma." << setfill('0') << setw(6) << myid;
   ...
   ParFiniteElementSpace fespace_nd(&mesh, &nd_fec);
   ParGridFunction dT(&fespace_nd);
   {
       ParDiscreteLinearOperator grad(&fespace, &fespace_nd);
...

   {
    ...
    ParaViewDataCollection paraview_dc("ex0p_potential", &mesh);
